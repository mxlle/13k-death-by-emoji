...
let speechBtn, voices, voiceListElement, solution, clickCounter = 0;
...
function init() {
    ...
    speechBtn = createElement({tag: 'button', text: '📢', cssClass: 'speak-button', onClick: () => speakEmojis()});
    document.body.appendChild(speechBtn);

    document.body.appendChild(createElement({tag: 'button', text: 'Reset', cssClass: 'reset-button', onClick: () => window.location.reload()}))
...
function getSolutionText() {
    ...
    for (let i = 0; i < shuffledEmojis.length; i++) {
        textParts.push(correctMatches[i] ? shuffledEmojis[i] : (i < clickCounter ? '❌' : '❓'));
    }
...
function endOfGame() {
    ...
    speechBtn.innerHTML = Math.round(correctCount / shuffledEmojis.length * 100) + '%';
    speechBtn.setAttribute('disabled', 'disabled');
}
